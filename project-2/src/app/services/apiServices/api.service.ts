...
import { Post, Comment, iMDB, iMDBRes, User, dataSearch } from 'src/types/api-types';
...
export class ApiService {
  ...
  login(email:string, password:string){
    ...
    const answer = this.http.post(environment.appUrl + '/users/login', { email, password })
    console.log(answer)
  ...
  register(email:string, password:string, rePassword:string, gender:string){
    ...
    const answer = this.http.post(environment.appUrl + '/users/register', { email, password, rePassword, gender })
    console.log(answer)
    return answer
  ...
  getRecords() {
    ...
    const answer= this.http.get<Post[]>(environment.appUrl + '/movies').pipe(
      tap((response) => {
 ...
 getItemById(id:string) {
  ...
  const res = this.http.get<Post>(environment.appUrl + '/movies/' + id).pipe(
    tap((response:any) => {
...
const res = this.http.get<Comment[]>(environment.appUrl + '/comments/' + id);
return res
}
...
addComment(data:Comment) {
  ...
  const response = this.http.post<Comment>(environment.appUrl + '/comments/', data);
  console.log(response)
...
imdbSearch(title:string){
  ...
  const response = this.http.get<iMDBRes>(`https://imdb8.p.rapidapi.com/auto-complete?q=${searching}`, {
	headers: {
		'X-RapidAPI-Key': '1f896608b2mshb3615bd17878f8dp1dcfe8jsncefc519ffd08',
...
getMyWishList(id:any) {
  ...
  const response = this.http.get<Post[]>(environment.appUrl+ `/movies/` + id);
  return response
}
...
searchFunction(data:dataSearch) {
    ...
    const response = this.http.post<Post[]>(environment.appUrl + '/search', data);
    return response
