...
exports.itemandYear1980to2020 = (item) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$gt: 1979, $lt: 2021}},
);


exports.itemandYear1980andOlder = (item) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$lt: 1980}},
);


exports.itemandYear2020andNewer = (item) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$gt: 2020}},
);


exports.itemandGenre = (item, genre) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "genre": { "$regex": `${genre}`, "$options": "i" }},
);
...
exports.itemGenreYear1980to2020 = (item, genres) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$gt: 1979, $lt: 2021},"genre": { "$regex": `${genres}`, "$options": "i" }},
);


exports.itemGenreYearear1980andOlder = (item, genres) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$lt: 1980}, "genre": { "$regex": `${genres}`, "$options": "i" }},
);

exports.itemGenreYear2020andNewer = (item, genres) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$gt: 2020}, "genre": { "$regex": `${genres}`, "$options": "i" }},
);

//---With all fields
exports.allYear1980to2020 = (item, genres, rpm) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$gt: 1979, $lt: 2021},"genre": { "$regex": `${genres}`, "$options": "i" }, "rpm": `${rpm}`},
);

exports.allYear1980andOlder = (item, genres, rpm) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$lt: 1980},"genre": { "$regex": `${genres}`, "$options": "i" }, "rpm": `${rpm}`},
);

exports.allYear2020andNwer = (item, genres, rpm) => Record.find(
    {"movieName": { "$regex": `${item}`, "$options": "i" }, "year": {$gt: 2020},"genre": { "$regex": `${genres}`, "$options": "i" }, "rpm": `${rpm}`},
);
...
exports.getSearchedbyArtistOrRecord = (item) => Record.find(
    {$or: [{ "artist": { "$regex": `${item}`, "$options": "i" } }, { "movieName": { "$regex": `${item}`, "$options": "i" } }]},
);
